...
(function () {
  ...
  function sanitizeFilename(name) {
    return String(name || 'case').replace(/[\\/:*?"<>|]/g, '_').trim() || 'case';
  }
  ...
  function bindGlobalEvents() {
    ...
    document.getElementById('btnDownloadHtml').addEventListener('click', () => {
      const c = getCurrentCaseForExport();
      if (c) downloadReadOnlyHtml(c);
    });
    document.getElementById('btnDownloadPdf').addEventListener('click', () => {
      const c = getCurrentCaseForExport();
      if (c) downloadReadOnlyPdf(c);
    });
  }

  function getCurrentCaseForExport() {
    const c = Storage.getById(state.currentCaseId);
    if (!c) { alert('먼저 케이스를 선택하거나 새로 만드세요.'); return null; }
    return c;
  }
  ...
  // ── 조회 전용 HTML / PDF 내보내기 ────────────────────────────
  // 편집용 폼(입력창)이 아니라 값만 나열한 별도의 읽기전용 렌더러를 써서
  // HTML 다운로드와 PDF 다운로드가 항상 같은 내용을 보여주게 한다.
  const READONLY_STYLE = `
    .ro-doc { font-family:'Noto Sans KR',sans-serif; color:#111; background:#fff; padding:32px; max-width:800px; margin:0 auto; }
    .ro-doc h1 { font-size:19px; margin-bottom:4px; }
    .ro-doc .ro-meta { font-size:11px; color:#666; margin-bottom:20px; }
    .ro-doc h2 { font-size:13px; margin:22px 0 8px; padding-bottom:5px; border-bottom:2px solid #222; }
    .ro-doc table { width:100%; border-collapse:collapse; margin-bottom:10px; font-size:12px; }
    .ro-doc th, .ro-doc td { border:1px solid #ccc; padding:6px 8px; text-align:left; vertical-align:top; }
    .ro-doc th { background:#f2f2f2; width:130px; font-weight:600; }
    .ro-doc td.r, .ro-doc th.r { text-align:right; }
    .ro-doc .ro-badges { display:flex; gap:12px; margin:12px 0 20px; flex-wrap:wrap; }
    .ro-doc .ro-badge { border:1px solid #999; border-radius:6px; padding:8px 14px; font-size:11px; min-width:90px; }
    .ro-doc .ro-badge b { display:block; font-size:16px; margin-top:2px; }
    .ro-doc .ro-check { color:#0a7a3d; } .ro-doc .ro-uncheck { color:#c23a3a; }
    .ro-doc ul { margin:0; padding-left:0; list-style:none; }
    .ro-doc .ro-doclist { columns:2; font-size:12px; line-height:1.9; }
    .ro-doc .ro-note { font-size:11px; color:#777; margin-top:10px; }

    /* 모바일: th 고정폭 표가 화면 밖으로 넘치지 않도록 행을 세로로 쌓는다 */
    @media (max-width:640px) {
      .ro-doc { padding:16px; }
      .ro-doc h1 { font-size:16px; }
      .ro-doc .ro-badges { gap:8px; }
      .ro-doc .ro-badge { min-width:0; flex:1 1 40%; }
      .ro-doc table, .ro-doc tbody, .ro-doc tr, .ro-doc th, .ro-doc td { display:block; width:auto !important; }
      .ro-doc th { border-bottom:none; padding-bottom:2px; }
      .ro-doc td { border-top:none; margin-bottom:4px; }
      .ro-doc .ro-doclist { columns:1; }
    }
  `;

  function buildReadOnlySummaryHtml(c, r) {
    const a = c.applicant, s = c.spouse, p = c.property, l = c.loan, cons = c.consult;
    const prefRows = c.preferentialItems.map(item => `
      <tr><td>${item.checked ? '<span class="ro-check">✔</span>' : '<span class="ro-uncheck">✕</span>'} ${escapeAttr(item.label)}</td><td class="r">${item.checked ? fmtPct(item.rate) : '—'}</td></tr>
    `).join('');
    const debtRows = c.existingDebts.length ? c.existingDebts.map(d => `
      <tr><td>${labelOf(DEBT_TYPES, d.type)}</td><td class="r">${fmtW(d.balance)}</td><td class="r">${fmtW(d.monthlyPayment)}</td><td>${labelOf(DEBT_METHODS, d.method)}</td></tr>
    `).join('') : '<tr><td colspan="4">등록된 기존 대출 없음</td></tr>';
    const docItems = c.requiredDocs.map(d => `
      <li>${d.checked ? '<span class="ro-check">☑</span>' : '<span class="ro-uncheck">☐</span>'} ${escapeAttr(d.label)}</li>
    `).join('');

    return `
      <div class="ro-doc">
        <h1>🏦 주택담보대출 상담 준비 — ${escapeAttr(c.name)}</h1>
        <div class="ro-meta">생성일 ${new Date(c.createdAt).toLocaleDateString('ko-KR')} · 최종수정 ${new Date(c.updatedAt).toLocaleDateString('ko-KR')} · 출력일시 ${new Date().toLocaleString('ko-KR')}</div>

        <div class="ro-badges">
          <div class="ro-badge">DSR 추정<b>${r.dsr == null ? '—' : fmtPct(r.dsr)}</b></div>
          <div class="ro-badge">LTV 추정<b>${r.ltv == null ? '—' : fmtPct(r.ltv)}</b></div>
          <div class="ro-badge">최종금리<b>${fmtPct(r.rate)}</b></div>
          <div class="ro-badge">대출원금<b>${fmtW(r.principal)}</b></div>
        </div>

        <h2>신청인 소득 · 근무</h2>
        <table>
          <tr><th>본인 연소득</th><td>${fmtW(a.income)}</td><th>근무유형</th><td>${labelOf(EMPLOYMENT_TYPES, a.employmentType)}</td></tr>
          <tr><th>재직/사업기간 등</th><td>${escapeAttr(a.employmentDetail) || '—'}</td><th>신용점수</th><td>${labelOf(CREDIT_BANDS, a.creditScoreBand)}</td></tr>
          ${s.included ? `
          <tr><th>배우자 연소득</th><td>${fmtW(s.income)}</td><th>배우자 근무유형</th><td>${labelOf(EMPLOYMENT_TYPES, s.employmentType)}</td></tr>
          <tr><th>배우자 재직/사업기간</th><td>${escapeAttr(s.employmentDetail) || '—'}</td><th>배우자 신용점수</th><td>${labelOf(CREDIT_BANDS, s.creditScoreBand)}</td></tr>
          ` : `<tr><th>배우자 소득 합산</th><td colspan="3">아니오</td></tr>`}
        </table>

        <h2>담보 물건 정보</h2>
        <table>
          <tr><th>주택 유형</th><td>${labelOf(PROPERTY_TYPES, p.type)}</td><th>매매·분양가</th><td>${fmtW(p.price)}</td></tr>
          <tr><th>감정가</th><td>${p.appraisedPrice ? fmtW(p.appraisedPrice) : '—'}</td><th>규제지역</th><td>${labelOf(REGULATION_ZONES, p.regulationZone)}</td></tr>
          <tr><th>보유주택 상태</th><td>${labelOf(OWNERSHIP_STATUS, p.ownershipStatus)}</td><th>세대주 여부</th><td>${p.isHouseholdHead ? '세대주' : '세대원'}</td></tr>
        </table>

        <h2>대출 조건</h2>
        <table>
          <tr><th>대출종류</th><td>${labelOf(LOAN_CATEGORY, l.category)}</td><th>대출목적</th><td>${labelOf(LOAN_PURPOSE, l.purpose)}</td></tr>
          <tr><th>대출은행</th><td>${escapeAttr(l.bank) || '—'}</td><th>희망 대출원금</th><td>${fmtW(l.principal)}</td></tr>
          <tr><th>기준금리</th><td>${labelOf(BASE_RATE_TYPES, l.baseRateType)} ${fmtPct(l.baseRate)}</td><th>가산금리</th><td>${fmtPct(l.spreadRate)}</td></tr>
          <tr><th>금리유형</th><td>${labelOf(RATE_TYPES, l.rateType)}</td><th>금리조정주기</th><td>${l.rateAdjustCycle}개월</td></tr>
          <tr><th>대출기간</th><td>${l.termYears}년</td><th>거치기간</th><td>${l.graceMonths}개월</td></tr>
          <tr><th>중도상환수수료율</th><td>${fmtPct(l.prepaymentFeeRate)}</td><th>면제조건</th><td>${escapeAttr(l.prepaymentFeeNote) || '—'}</td></tr>
        </table>

        <h2>기존 대출 (DSR 산정용)</h2>
        <table><tr><th>종류</th><th class="r">잔액</th><th class="r">월상환액</th><th>상환방식</th></tr>${debtRows}</table>

        <h2>우대금리 체크리스트</h2>
        <table><tr><th>항목</th><th class="r">적용금리</th></tr>${prefRows}</table>

        <h2>정책·규제 기준일</h2>
        <table>
          <tr><th>분양(매매) 계약일</th><td>${c.contractDate || '—'}</td></tr>
          <tr><th>규제 관련 메모</th><td>${escapeAttr(c.policyNote) || '—'}</td></tr>
        </table>

        <h2>상담 관리</h2>
        <table>
          <tr><th>지점명</th><td>${escapeAttr(cons.branch) || '—'}</td><th>담당자 연락처</th><td>${escapeAttr(cons.contactPhone) || '—'}</td></tr>
          <tr><th>상담 예정일시</th><td colspan="3">${cons.scheduledAt || '—'}</td></tr>
          <tr><th>메모</th><td colspan="3">${escapeAttr(cons.memo) || '—'}</td></tr>
        </table>

        <h2>준비서류 체크리스트</h2>
        <ul class="ro-doclist">${docItems}</ul>

        <h2>상환방식 비교</h2>
        <table>
          <tr><th>방식</th><th class="r">첫달 납입액</th><th class="r">마지막달 납입액</th><th class="r">총이자</th><th class="r">총상환액</th></tr>
          <tr><td>원리금균등</td><td class="r">${fmtW1(r.eq.firstMonthPayment)}</td><td class="r">${fmtW1(r.eq.lastMonthPayment)}</td><td class="r">${fmtW1(r.eq.totalInterest)}</td><td class="r">${fmtW1(r.eq.totalPayment)}</td></tr>
          <tr><td>원금균등</td><td class="r">${fmtW1(r.ep.firstMonthPayment)}</td><td class="r">${fmtW1(r.ep.lastMonthPayment)}</td><td class="r">${fmtW1(r.ep.totalInterest)}</td><td class="r">${fmtW1(r.ep.totalPayment)}</td></tr>
          <tr><td>체증식(근사)</td><td class="r">${fmtW1(r.su.firstMonthPayment)}</td><td class="r">${fmtW1(r.su.lastMonthPayment)}</td><td class="r">${fmtW1(r.su.totalInterest)}</td><td class="r">${fmtW1(r.su.totalPayment)}</td></tr>
        </table>
        <p class="ro-note">※ DSR·LTV·체증식 결과는 참고용 추정치이며 실제 은행 심사 결과와 다를 수 있습니다. 이 문서는 이 브라우저에만 저장된 데이터를 바탕으로 로컬에서 생성되었습니다.</p>
      </div>
    `;
  }

  function downloadReadOnlyHtml(c) {
    const r = computeCase(c);
    const doc = `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${escapeAttr(c.name)} - 주택담보대출 상담 자료</title><style>${READONLY_STYLE}</style></head><body>${buildReadOnlySummaryHtml(c, r)}</body></html>`;
    const blob = new Blob([doc], { type: 'text/html' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${sanitizeFilename(c.name)}-상담자료.html`;
  ...
  async function downloadReadOnlyPdf(c) {
    if (typeof html2canvas === 'undefined' || !window.jspdf) {
      alert('PDF 생성 라이브러리를 불러오지 못했습니다. 인터넷 연결 상태를 확인해주세요.');
      return;
    }
    ...
    container.innerHTML = `<style>${READONLY_STYLE}</style>${buildReadOnlySummaryHtml(c, r)}`;
    document.body.appendChild(container);

    try {
      await new Promise(resolve => setTimeout(resolve, 50)); // 레이아웃 반영 대기
      ...
      pdf.save(`${sanitizeFilename(c.name)}-상담자료.pdf`);
    } catch (err) {
      console.error('[mortgage] PDF 생성 실패', err);
